terraform {
  backend "s3" {
    ...
    key    = "production-state" # update this to your state file name
  ...
  # backend "gcs" {
  #   bucket = "my-tf-state-bucket" # update this to your bucket name
  #   prefix = "production-state" # update this to your state file prefix
  # }
...
resource "random_string" "prod_random" {
  length = 16
  special = true
  override_special = "/@£$"
}
